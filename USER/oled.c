...
void OLED_Refresh_Line(uint8_t y)
{
	...
	{  
		...
		OLED_WR_Byte (0x00,OLED_CMD);      //设置显示位置—列低地址
		OLED_WR_Byte (0x10,OLED_CMD);      //设置显示位置—列高地址   
			OLED_WR_Byte(OLED_GRAM[y][i],OLED_DATA); 
	}   
}
...
void OLED_PutPixel(unsigned char x,unsigned char y)
{
 ...
 OLED_WR_Byte((unsigned char)(0xb0+(y>>3)),OLED_CMD);
 OLED_WR_Byte((unsigned char)(((x&0xf0)>>4)|0x10),OLED_CMD);
 OLED_WR_Byte((unsigned char)((x&0x0f)|0x00),OLED_CMD);
...
void LCD_DrawLine(unsigned int x1, unsigned int y1, unsigned int x2,unsigned int y2)
{
	...
	{  
		OLED_DrawPoint(uRow,uCol,1);//画祦E
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
